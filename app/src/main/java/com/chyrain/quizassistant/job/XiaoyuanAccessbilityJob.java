...
/**
 * 优酷《疯狂夺金》AccessibilityEvent和通知事件接收处理
 * @author Chyrain
 ...
 */
public class XiaoyuanAccessbilityJob extends DatiAccessbilityJob {

    private static final String TAG = "XiaoyuanAccessbilityJob";

    /** 监听应用的包名(需添加到——service_config.xml) */
    private static final String THE_PACKAGENAME = "com.fenbi.android.solar";
    private static final String THE_JOBKEY = "qf";

    /** 红包消息的关键字*/
    private static final String APP_NAME = "知识英雄";
    ...
    public boolean isEnable() {
        return getConfig().isEnableWechat() && getConfig().isEnableZSYX();
    }
    ...
    public void onReceiveJob(AccessibilityEvent event) {
         ...
         */
            ...
            if (event.getClassName().equals("com.fenbi.android.solar.activity.HomeActivity")) {
                ...
                if (getConfig().isEnableAutoTrust()) {
                    // 进入答题页面 dynamic_container ->  clickable的Fragment
                    mHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            clickAtNodeWithId("bottom_banner");
                        }
                    }, 2000);
                }
            } else if (event.getClassName().equals("com.live.puzzle.ui.MainActivity")) {
                mLastWindow = mCurrentWindow;
                mCurrentWindow = WINDOW_READY_PAGE;
                Logger.i(TAG, "mCurrentWindow:" + mCurrentWindow + " activity=>" + event.getClassName());
                ...
                if (getConfig().isEnableAutoTrust()) {
                    mHandler.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            // 进入答题页面
                            clickAtNodeWithId("btnClock");
                        }
                    }, 2000);
                }
            } else if (event.getClassName().equals("com.live.puzzle.ui.LiveActivity")) {
                mLastWindow = mCurrentWindow;
